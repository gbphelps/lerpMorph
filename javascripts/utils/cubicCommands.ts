...
import { AbsoluteCommand, AnyCommand } from '../types';
...
export default function cubicCommands(commands: AbsoluteCommand[]) {
  const cubics: AnyCommand[] = [];
  ...
  for (let i = 0; i < commands.length; i++) {
    ...
    if (command.type === 'A') {
      const prev = commands[i - 1].params;
    ...
    if (command.type === 'L' || command.type === 'Q') {
      ...
      for (let i = 0; i < command.params.length / 2; i++) {
        points.push({
      ...
      // NOTE: now we strip the previous point by starting at i=1,
      // because it's not part of the path definition
